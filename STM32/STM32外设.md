...
我们可以将最终传至NVIC的中断类型分为三种
1. 输入线设定为GPIO/AFIO
2. 输入线设定为事件
3. 手动修改软件中断事件寄存器
输入线通过寄存器设定为某个GPIO/AFIO或某个事件，输入的通常为一存在电平变化的信号，输入到边沿检测电路，该电路由上升沿触发选择寄存器和下降沿触发选择寄存器配置该电路由上升沿还是下降沿触发中断标志位0/1，继而将标志位输入3结构中的“或门”电路，与标志位并行的

综上，我们可以看出边沿检测电路的作用是用于检测外部中断/事件，3处“与门”电路适用于检测中断产生与否，4处“或门”电路用于屏蔽事件，1处“或门”电路用于屏蔽中断
